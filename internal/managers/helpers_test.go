...
func progressAt(tb testing.TB, conn adbc.Connection, arrival, commit time.Time) {
	tb.Helper()
	// The shape core.ProgressStore.Init creates, including the column a
	// source's delivering answer lands in. NULL here: these tests describe a
	// source that never said, which reads as delivering.
	exec(tb, conn, `CREATE TABLE IF NOT EXISTS sqlflow_progress (last_arrival TIMESTAMPTZ, last_commit TIMESTAMPTZ, delivering_for_us BIGINT, messages BIGINT NOT NULL)`)
	exec(tb, conn, `DELETE FROM sqlflow_progress`)
	exec(tb, conn, fmt.Sprintf(`INSERT INTO sqlflow_progress VALUES (TIMESTAMPTZ '%s', TIMESTAMPTZ '%s', NULL, 0)`,
