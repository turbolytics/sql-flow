...
func TestStateDurability_TheRowSaysTheStreamWokeBeforeItsRowsAppear(t *testing.T) {
	...
	// And the silence after the burst still counts, which is what makes the
	// bucket close at all. Suppressing the quiet before a batch must not
	// suppress the quiet after it: the arrival clock is left at this batch,
	// so every idle tick from here confirms the time since it. A fix that
	// held the quiet at zero would trade a bucket that closes early for one
	// that never closes, and the counting property cannot tell those apart.
	beforeTicks := time.Now()
	time.Sleep(20 * time.Millisecond)
	assert.NoError(t, tb.recordProgress(ctx, progressForced))
	first := quietInRow(t, reader)

	time.Sleep(20 * time.Millisecond)
	assert.NoError(t, tb.recordProgress(ctx, progressForced))
	second := quietInRow(t, reader)

	assert.That(t, first >= 20*time.Millisecond)
	assert.That(t, second > first)
	// Measured from this batch, not from the silence before it: the 34
	// seconds the burst ended are gone for good.
	assert.That(t, second < time.Since(beforeTicks)+time.Second)
}
