...
- A windowed pipeline with `idle_close_seconds` and no `state.path` could
  ...
  be seen: one extra progress write per waking, none for a stream whose
  batches are less than a second apart, and none for a pipeline with a state
  path, where the rows and the row already commit together. A second is the
  threshold because `idle_close_seconds` is whole seconds, so no window can
  close on a silence shorter than one.
