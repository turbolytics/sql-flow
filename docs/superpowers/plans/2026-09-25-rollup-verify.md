...
- **No `degraded` (after the PR opened).** The maintainer ruled that `rollup run` is a harness, a provisioner, backfiller and reporter, so `/healthz` reports the process and not the data. Drift is counted in `rollup_drift_buckets` and logged, and `/healthz` stays `healthy`. `DriftIsReportedAndClears` became `DriftIsCountedAndHealthStaysHealthy`, and the spec's `/healthz` table loses its `degraded` row. Everywhere above that says drift turns `/healthz` to `degraded` is superseded.
- **A table name collision (final review).** Verify's own columns were `side`, `in_want` and `in_got`, so a dimension with one of those names made the statement ambiguous. They are now `"~side"`, `"~in_want"` and `"~in_got"`, pinned by `VerifyAcceptsDimensionsNamedLikeItsOwnColumns`.
