...
**What already carries over.** The model's counting property, now able to fail
on a bucket that never closes (`0df50fb`); its ability to place a poll inside a
batch (`4052dd3`); its check that a row agrees with the instants it was computed
from (`631606d`); the clock and trigger seams (`core.WithClock`,
...
list is checked against the run's own decisions, not the quiescing tail
(`631606d`), so a shape under which the idleness rule cannot fire fails
